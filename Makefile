# Scripts to handle ipseity build and installation
...
.PHONY: all install build ipseity deps test citest clean doc protobuf
...
install: build
	$(info $(GM) installing ipseity and making configuration …)
	@ cp $(BUILD)/ipseity /usr/local/bin/

# Build the various binaries and sources
build: protobuf ipseity

# Build the ipseity command and store in the build directory
ipseity:
	$(info $(GM) compiling ipseity executable …)
	@ $(GOBUILD) -o $(BUILD)/ipseity ./cmd/ipseity
...
doc:
	...
	@ open http://localhost:6060/pkg/github.com/bbengfort/ipseity/
	@ $(GODOC) --http=:6060
