...
def draw_benchmark(path):
    ...
    ax[0].fill_between(np.arange(1, len(means)+1), means+std, means-std, alpha=0.25)

    ax[0].set_xlim(1,len(means)+1)
    ax[0].set_ylabel("requests/second")
    ax[0].set_xlabel("concurrent clients")
    ax[0].set_title("Ipseity Benchmark")
    ax[0].legend(frameon=True)

    sns.barplot('clients', 'throughput', ax=ax[1], data=df, palette="GnBu_d")
    ax[1].set_ylabel("")
    ax[1].set_xlabel("concurrent clients")
    ax[1].set_title("Ipseity Benchmark")
